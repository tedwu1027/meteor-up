...
Actions.prototype.ls = function() {
  this._showKadiraLink();
  this._executePararell("ls", [this.config]);
};

Actions.prototype.revert = function() {
  if (!process.argv[3]){
    console.error('Error: Version is not defined.')
    process.exit(1);
  }

  this._showKadiraLink();
  this._executePararell("revert", [this.config, process.argv[3]]);
};
      ...
      async.mapSeries(
        ...
        function (sessionData, callback) {
          ...
          var taskList = taskListsBuilder.deploy(
            bundlePath, env,
            deployCheckWaitTime, appName, enableUploadProgressBar, process.argv[3]);
