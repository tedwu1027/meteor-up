...
exports.ls = function(config) {
  var appName  = config.appName;
  var taskList = nodemiral.taskList('ls current and all tags (linux)');
  // ls
  taskList.executeScript('ls', {
    script: path.resolve(TEMPLATES_DIR, 'ls.sh'),
    vars: {
      appName: appName,
    }
  });
...
exports.deploy = function(bundlePath, env, deployCheckWaitTime, appName, enableUploadProgressBar, version) {
  var taskList = nodemiral.taskList("Deploy app '" + appName + "' to v" + version + "' (linux)");
  ...
  taskList.executeScript('Create v' + version + ' directory', {
    script: path.resolve(TEMPLATES_DIR, 'mkdir.sh'),
    vars: {
      appName: appName,
      version: version
    }
  ...
  taskList.copy('Setting up Environment Variables', {
    ...
    dest: '/opt/' + appName + '/tags/' + version + '/config/env.sh',
    vars: {
      env: env || {},
      appName: appName
    }
...
exports.revert = function(env, version) {
  var taskList = nodemiral.taskList("Revert app '" + appName + "' to v" + version + " (linux)");
  var appName  = env.appName;
  var deployCheckWaitTime = env.deployCheckWaitTime;

  // deploying
  taskList.executeScript('Revert to old version', {
    script: path.resolve(TEMPLATES_DIR, 'revert.sh'),
    vars: {
      deployCheckWaitTime: deployCheckWaitTime || 10,
      appName: appName,
      version: version
